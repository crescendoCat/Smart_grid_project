function train_origin(DATA_PATH, quoted_range, buy_range, sup_range, usr_range, ...
...
fprintf('Min plant supply: %.2f\n', min(plants_data_base(:)));
fprintf('Max user need: %.2f\n', max(total_power_dem(:)));
fprintf('Min user need: %.2f\n', min(total_power_dem(:)));
...
sup_Q_factor = zeros(plant_num, total_state, supply_action_num);
usr_Q_factor = zeros(buy_num, total_state, user_action_num);

% Total reward and average reward table
sup_tr = zeros(plant_num, total_state, supply_action_num);
usr_tr = zeros(buy_num, total_state, user_action_num);
sup_ar = zeros(plant_num, total_state, supply_action_num);
usr_ar = zeros(buy_num, total_state, user_action_num);
...
sup_weight = ones(plant_num, total_state, supply_action_num);
usr_weight = ones(buy_num, total_state, user_action_num);
...
for day = 1:day_num
    ...
    for compute_time = 7:18
        fprintf('Total supply in day %d hour %d: %.2f\n', day, compute_time, ...
            sum(plants_data(compute_time, :)));
        fprintf('Total demand in day %d hour %d: %.2f\n', day, compute_time, ...
            sum(power_dem(compute_time, :)));
        % Convert current demand to discrete state
        sup_cur_state = discretize(plants_data(compute_time, :), supply_state_edges);
        dem_cur_state = discretize(power_dem(compute_time, :), demand_state_edges);
        ...
        for iter_ = 1:ITERMAX 
            ...
            for j = 1:plant_num
                % Get the state
                cur_state = total_dem_cur_state * supply_state_num + sup_cur_state(j);              
                ...
                % update total reward
                sup_tr(j,cur_state,quoted_p) = sup_tr(j,cur_state,quoted_p) + immi_reward;  
                % update weight matrix
                sup_weight(j, cur_state, quoted_p) = sup_weight(j, cur_state, quoted_p) + 1;                
                % Get average reward
                sup_ar(j,cur_state,quoted_p) = (1-beta)*sup_ar(j,cur_state,quoted_p) ...
                    + beta*(sup_tr(j,cur_state,quoted_p)/sup_weight(j, cur_state, quoted_p));
                avg_reward = sup_ar(j,cur_state,quoted_p);
                % Get next state
                next_state = discretize(x(j+buy_num), demand_state_edges);
                if strcmp(update_method, 'R-SMART')
                % Update supply Q-factor (R-SMART)
                
                sup_Q_factor(j,cur_state,quoted_p) = ...
                    (1-lr)*sup_Q_factor(j, cur_state, quoted_p)+ ...
                    lr*(immi_reward - avg_reward + eta*(max(sup_Q_factor(j,next_state,:))));
                %{
                sup_Q_factor(j,cur_state,quoted_p) = ...
                    sup_Q_factor(j,cur_state,quoted_p) + immi_reward;
                %}
            ...
            for j = 1:buy_num
                % Get the state
                cur_state = total_sup_cur_state * demand_state_num + dem_cur_state(j);
                % Get the immidiate reward
                immi_reward = (x(j)/power_dem(compute_time, j))*(1-(buy_price(j)/buy_price_ub));
                % Get quoted price -> action num
                buy_p = buy_price(j) - buy_price_lb + 1;
                % update total reward
                usr_tr(j,cur_state,buy_p) = usr_tr(j,cur_state,buy_p) + immi_reward;  
                % update weight matrix
                usr_weight(j, cur_state, buy_p) = usr_weight(j, cur_state, buy_p) + 1; 
                % Get average reward
                usr_ar(j,cur_state,buy_p) = (1-beta)*usr_ar(j,cur_state,buy_p) ...
                    + beta*(usr_tr(j,cur_state,buy_p)/usr_weight(j, cur_state, buy_p));
                avg_reward = usr_ar(j,cur_state,buy_p);
                % Get next state
                next_state = discretize(x(j), supply_state_edges);
                if strcmp(update_method, 'R-SMART')
                % Update user Q-factor (R-SMART)
                
                usr_Q_factor(j,cur_state,buy_p) = ...
                    (1-lr)*usr_Q_factor(j, cur_state,buy_p)+ ...
                    lr*(immi_reward - avg_reward + eta*(max(usr_Q_factor(j,next_state,:))));
                %{
                usr_Q_factor(j,cur_state,buy_p) = ...
                    usr_Q_factor(j,cur_state,buy_p) + immi_reward;
                %}
                elseif strcmp(update_method, 'Q-learning')
