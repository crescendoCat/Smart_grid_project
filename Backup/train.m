...
for day = 1:day_num
    ...
    for compute_time = 7:18
        ...
        %sup_cur_state = discretize(sum(power_dem(compute_time, :)), demand_state_edges)*ones(plant_num, 1);
        ...
        for iter_ = 1:ITERMAX 
            ...
            for j = 1:plant_num    
                ...
                % update total reward
                sup_tr(j,sup_cur_state(j),quoted_p) = sup_tr(j,sup_cur_state(j),quoted_p) + immi_reward;  
                % update weight matrix
                sup_weight(j, sup_cur_state(j), quoted_p) = sup_weight(j, sup_cur_state(j), quoted_p) + 1;                
                % Get average reward
                sup_ar(j,sup_cur_state(j),quoted_p) = (1-beta)*sup_ar(j,sup_cur_state(j),quoted_p) ...
                    + beta*(sup_tr(j,sup_cur_state(j),quoted_p)/sup_weight(j, sup_cur_state(j), quoted_p));
                avg_reward = sup_ar(j,sup_cur_state(j),quoted_p);
                % Get next state
                next_state = discretize(x(j+buy_num), demand_state_edges);
                if strcmp(update_method, 'R-SMART')
                % Update supply Q-factor (R-SMART)
                %{
                sup_Q_factor(j,sup_cur_state(j),quoted_p) = ...
                    (1-lr)*sup_Q_factor(j, sup_cur_state(j), quoted_p)+ ...
                    lr*(immi_reward - avg_reward + eta*(max(sup_Q_factor(j,next_state,:))));
                %}
                sup_Q_factor(j,sup_cur_state(j),quoted_p) = ...
                    sup_Q_factor(j,sup_cur_state(j),quoted_p) + immi_reward;
            ...
            for j = 1:buy_num
                ...
                % update total reward
                usr_tr(j,dem_cur_state(j),buy_p) = usr_tr(j,dem_cur_state(j),buy_p) + immi_reward;  
                % update weight matrix
                usr_weight(j, dem_cur_state(j), buy_p) = usr_weight(j, dem_cur_state(j), buy_p) + 1; 
                % Get average reward
                usr_ar(j,dem_cur_state(j),buy_p) = (1-beta)*usr_ar(j,dem_cur_state(j),buy_p) ...
                    + beta*(usr_tr(j,dem_cur_state(j),buy_p)/usr_weight(j, dem_cur_state(j), buy_p));
                avg_reward = usr_ar(j,dem_cur_state(j),buy_p);
                % Get next state
                next_state = discretize(x(j), supply_state_edges);
                if strcmp(update_method, 'R-SMART')
                % Update user Q-factor (R-SMART)
                %{
                usr_Q_factor(j,dem_cur_state(j),buy_p) = ...
                    (1-lr)*usr_Q_factor(j, dem_cur_state(j),buy_p)+ ...
                    lr*(immi_reward - avg_reward + eta*(max(usr_Q_factor(j,next_state,:))));
                %}
                usr_Q_factor(j,dem_cur_state(j),buy_p) = ...
                    usr_Q_factor(j,dem_cur_state(j),buy_p) + immi_reward;
